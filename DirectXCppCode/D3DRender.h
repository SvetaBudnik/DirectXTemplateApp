...
//		ID3D11Texture2D* pBackBuffer = NULL;
		...
		CheckHR(CreateDDSTextureFromFile(device, L"..\\Texture1.dds", NULL, &textureView));
	...
	RenderingUnit CreateTriangleColorUnit(std::vector<std::array<std::array<int, 3>,2>>& Triangles, std::vector<std::array<double, 5>>& vertexes, std::vector<std::array<double, 3>>& normals) //в паре первое -- треугольник, второе -- rgb цвет
	{
		RenderingUnit unit;
		...
		for(int i=0;i<vertexes.size();i++)
		{
			...
			vertarray[i].Color.x= vertexes[i][3];
			vertarray[i].Color.y = vertexes[i][4];
			//_______________________________________
